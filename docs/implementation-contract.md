...
The live activity stream must make agent work inspectable without exposing private prompts or hidden reasoning. Emit run and node lifecycle, safe tool-call lifecycle, evidence discoveries, calculation completion, review interrupts, revision invalidation and artifact generation. Each displayed tool event includes a stable call ID, agent role, tool name, human label, status, start/end time, latency, redacted input summary, bounded output summary, evidence/artifact references and error class. Do not stream raw document bodies, complete tool arguments, model chain-of-thought, credentials or provider payloads.

Use AG-UI tool-call events for model-requested tools and custom events for deterministic workflow nodes. Publish at least: `RUN_STARTED`, `NODE_STARTED`, `TOOL_CALL_STARTED`, `TOOL_CALL_COMPLETED`, `TOOL_CALL_FAILED`, `EVIDENCE_FOUND`, `DEFINITION_RESOLVED`, `CALCULATION_COMPLETED`, `REVIEW_REQUIRED`, `RESULT_INVALIDATED`, `APPROVAL_INVALIDATED`, `PACKAGE_REVISED`, `RUN_COMPLETED` and `RUN_FAILED`. A node or tool completion event is persisted before it is shown as complete. Progress is derived from completed required stages, never invented token-by-token.
